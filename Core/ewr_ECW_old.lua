...
local function unitConversionOutput(unitName,bearing,distance,altitude,dir,targetTypeName)
	...
	if altWord == "Metric" then
		...
		if dist >= 3 then dist = tostring(math.floor(dist))
		else dist = string.format("%1.1f", dist) end
		if alt >= 1 then alt = tostring(util.round(alt,1))
		else alt = string.format("%1.1f", alt) end
	...
	elseif altWord == "Imperial" then
		if (altitude * mToFt) / 1000 < 1 then altWord = "Cherubs" altitude = altitude * 10
		else altWord = "Angels" end
		local dist = ((distance/1000) * kmToMi) / miToNm
		local alt = (altitude * mToFt) / 1000
		if dist >= 1.61987 then dist = tostring(math.floor(dist))
		else dist = string.format("%1.1f", dist) end
		if alt >= 1 then alt = tostring(math.floor(alt))
		else alt = string.format("%1.1f", alt) end
			...
			for unitKey, unitValues in next, ewr.getDetectedObjects(ewrName) do
				if unitValues.object ~= nil then
					if unitValues.object:inAir() and unitValues.object:getDesc().category == Unit.Category.AIRPLANE then
						if unitValues.object:getCoalition() ~= Unit.getByName(ewrName):getCoalition() then
	...
	for k, v in pairs(list) do
		...
		if target ~= nil then
			if target:isExist() then
				if target:getDesc().category == 0 and target:getCoalition() ~= unit:getCoalition() and target:getName() ~= unit:getName() then
					local bearing = ewr.bearingUnit(unit,target)
					local altitude = target:getPoint().y
					local distance = v[2]
					local direction =  bearing - heading(target:getName())
...
function ewr.bomber_picture(unitName)
	...
	for k, v in pairs(list) do
		...
		if target ~= nil then
			if target:isExist() then
				if target:getDesc().category == 0 and target:getCoalition() ~= unit:getCoalition() and target:getName() ~= unit:getName() then
					if type[target:getTypeName()] == true then
						local bearing = ewr.bearingUnit(unit,target)
						local altitude = target:getPoint().y
						local distance = v[2]
						local direction =  bearing - heading(target:getName())
...
function ewr.friendlyPicture(unitName)
	...
	for k, v in pairs(list) do
		...
		if playerName ~= ", callsign nil" then
			output = output .. target:getPlayerName() ..", " .. unitConversionOutput(unitName,bearing,distance,altitude,dir,target:getTypeName()) .. "\n"
...
function ewrEventHandler:onEvent(event)
	...
	if world.event.S_EVENT_BIRTH == event.id then
		...
		if ewr.hasCommands[event.initiator:getGroup():getID()] == nil then
			...
			missionCommands.addCommandForGroup(event.initiator:getGroup():getID() , "Request Bomber Intercepts" , subMenu , ewr.bomber_picture , event.initiator:getName())
